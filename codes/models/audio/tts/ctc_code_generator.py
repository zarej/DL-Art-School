...
class CtcCodeGenerator(nn.Module):
    def __init__(self, model_dim=512, layers=10, max_length=2048, dropout=.1, ctc_codes=256, max_pad=120, max_repeat=30):
        ...
        # nn.Embedding
        self.position_embedding = mbnb.nn.Embedding(max_length, model_dim)
        # nn.Embedding
        self.codes_embedding = mbnb.nn.Embedding(ctc_codes, model_dim)
        # nn.Embedding
        self.recursive_embedding = mbnb.nn.Embedding(pred_codes, model_dim)
        ...
        self.pred_head = mbnb.nn.Linear(model_dim, pred_codes)
        self.confidence_head = mbnb.nn.Linear(model_dim, 1)
